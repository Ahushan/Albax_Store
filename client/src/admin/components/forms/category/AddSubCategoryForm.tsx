...
export default function CreateSubCategory() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      toast.success(`SubCategory created: ${data.name}`);
      reset({ name: "", categoryId: "" });
    } catch (error: any) {
      toast.error(error?.response?.data?.message || "SubCategory not created");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <motion.form
      ...
      onKeyDown={(e) => {
        // Prevent Enter key from submitting form when selecting category
        if (
          e.key === "Enter" &&
